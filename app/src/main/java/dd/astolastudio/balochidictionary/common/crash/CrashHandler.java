package dd.astolastudio.balochidictionary.common.crash;
				...
				private void tryUncaughtException(Thread thread, Throwable throwable) {
					...
					String fullStackTrace; {
...

						{
							...
							try {
								List<String> m=App.getLogger();
								if (!m.isEmpty()) {
									pw.println("************ More Details ***************\n");
							...
							try {
								...
								Set<Thread> threads=m.keySet();
								pw.println("*********** Thread Details **************\n");

								for (Thread mThread: threads) {
									if (!Thread.currentThread().equals(mThread)) {
										StackTraceElement[] stackTrace = m.get(mThread);
										if (stackTrace != null && stackTrace.length > 0) {
											pw.println(mThread.toString());
						...
						pw.close();
						//fullStackTrace=Log.getStackTraceString(throwable);
					}
